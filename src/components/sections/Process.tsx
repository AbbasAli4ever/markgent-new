...
export default function Process() {
    ...
    const ctx = gsap.context(() => {
      mm.add("(min-width: 1280px)", () => {
        STEPS.forEach((_, i) => {
          ...
          gsap.set(card, { opacity: 0, y: direction * 24, transformOrigin: "50% 50%" });
          if (lineRefs.current[i]) gsap.set(lineRefs.current[i], { scaleX: 0, transformOrigin: "0% 50%" });
        });

        const tl = gsap.timeline({ scrollTrigger: { trigger: section, start: "top top", end: `+=${STEPS.length * 500}`, scrub: 0.6, pin: true, anticipatePin: 1 } });

        STEPS.forEach((_, i) => {
          tl.to(stepRefs.current[i], { opacity: 1, scale: 1, duration: 1, ease: "back.out(1.8)" }, i === 0 ? 0 : ">-0.2")
            .to(cardRefs.current[i], { opacity: 1, y: 0, duration: 1, ease: "power2.out" }, "<")
            .to(lineRefs.current[i] ?? [], { scaleX: 1, duration: 1, ease: "power2.out" }, "<");
        });
  ...
  return (
    <section
      ref={sectionRef}
      id="process"
      className="mt-16 overflow-hidden bg-ink text-cream xl:mt-24 xl:min-h-screen xl:min-h-[100svh]"
    >
      <div className="mx-auto flex max-w-[1240px] flex-col px-5 py-16 sm:px-8 sm:py-20 xl:min-h-screen xl:min-h-[100svh] xl:px-10 xl:py-22">
        <div className="flex items-end justify-between gap-10 flex-wrap">
          <div ref={headingRef} className="max-w-[600px]">
            <div className="text-[12.5px] tracking-[0.16em] uppercase text-cream/55">
              Our Process
            </div>
            <h2 className="mt-3.5 text-[38px] leading-[1.08] text-paper sm:text-[46px] xl:text-[54px]">
        ...
        <div className="mt-24 hidden flex-1 grid-cols-5 items-center py-24 xl:grid">
          {STEPS.map((step, i) => {
            const isBelow = i % 2 === 0;
            return (
        ...
        <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 xl:hidden">
          {STEPS.map((step) => (
